# elasticsearch 自带的搜索建议功能
# elasticsearch 文档  https://www.elastic.co/guide/en/elasticsearch/guide/current/index.html



# test_scrapy_spider\test_scrapy_spider\models\es_types.py
# -*- coding: utf-8 -*-
...
from elasticsearch_dsl.connections import connections  #连接到es
...
def gen_suggests(index, info_tuple):  #用tuple可以传递多个值
    #根据字符串生成搜索建议数组
    used_words = set()  # 去重，不能让后面分词分出来的权重把前面的覆盖了
    suggests = []  #最后返回的suggests
    for text, weight in info_tuple:
        if text:  #如果为空字符串，就不处理
            #调用es的analyze接口分析字符串  #将字符串分词，做大小写转换
            words = es.indices.analyze(index=index, analyzer="ik_max_word", params={'filter':["lowercase"]}, body=text)  #生成的words是{}  #生成的words["tokens"]是一个[{}，{}，{}]
            anylyzed_words = set([r["token"] for r in words["tokens"] if len(r["token"])>1])  #len(r["token"])>1用来过滤单个字的分词，汉语中单个字一般没有含义
            new_words = anylyzed_words - used_words  #去重  #set相减
        else:
            new_words = set()

        if new_words:
            suggests.append({"input":list(new_words), "weight":weight})  # [{"input":[]},{"weight":2}]  #按照格式
...
# """
    ...
    def save_to_es(self):
        ...
        article.suggest = gen_suggests(ArticleType._doc_type.index, ((article.title,10),(article.tags, 7)))  # 设置搜索建议的值
